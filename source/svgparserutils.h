...
#define IS_NAMECHAR(c) \
    (IS_ID(c) || IS_NUM(c) || c=='-' || c=='.')

#define IS_CSSID(c) \
    (IS_ALPHA(c) || c=='_')

#define IS_CSSNAMECHAR(c) \
    (IS_ALPHA(c) || IS_NUM(c) || c=='_' || c=='-')

namespace lunasvg {

namespace Utils {

inline bool skipDesc(const char*& ptr, const char* str, size_t len)
{
    const char* start = ptr;
    while(len && *ptr && *str && *ptr==*str)
    {
        ++ptr;
        ++str;
        --len;
